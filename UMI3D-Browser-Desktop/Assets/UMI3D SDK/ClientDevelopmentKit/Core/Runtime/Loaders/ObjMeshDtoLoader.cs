...
namespace umi3d.cdk
{
    ...
    public class ObjMeshDtoLoader : AbstractMeshDtoLoader
    {
        ...
        ///<inheritdoc/>
        public override void UrlToObject(string url, string extension, string authorization, Action<object> callback, Action<Umi3dExcption> failCallback, string pathIfObjectInBundle = "")
        {
                ...
                UMI3DEnvironmentLoader.Instance.GetBaseMaterialBeforeAction(
                    (m) =>
                    {
                        ...
                        objImporter.ImportError += (s) => {
                            failed = true;
                            failCallback(new Umi3dExcption(401,$"Importing failed for : {url}")); };
                        ...
                        objImporter.ImportingComplete += () =>
                        {
                            if (!failed)
                            {
                                ...
                                {
                                    failCallback(new Umi3dExcption(0, $"Importing completed but callback failed for : {url} {e}"));
                                }
